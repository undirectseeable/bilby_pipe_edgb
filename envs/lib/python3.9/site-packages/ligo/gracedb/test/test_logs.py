...
@pytest.mark.parametrize(
    "is_event,obj_id,tag_name,display_name,label",
    [
        (True, "T123456", None, None, None),
        (False, "TS121212abc", None, None, None),
        (True, "T123456", 'test', None, None),
        (True, "T123456", 'test', None, 'LABEL_NAME'),
        (True, "T123456", ('new_tag',), None, None),
        (True, "T123456", ['tag1', 'tag2'], None, None),
        (False, "TS121212abc", 'tag1', 'tag1_disp', None),
        (False, "TS121212abc", ['tag1'], 'tag1_disp', None),
        (False, "TS121212abc", set(['tag1']), set(['tag1_disp']), None),
    ]
)
def test_write_log(safe_client, is_event, obj_id, tag_name,
                   display_name, label):
    # Comment
    comment = 'test'
    ...
    with mock.patch('ligo.gracedb.rest.GraceDb.post') as mock_post, \
         ...
         mock.patch(template_prop, mock_template_dict):  # noqa: E127
        safe_client.write_log(obj_id, comment, tag_name=tag_name,
                              displayName=display_name, label=label)
    ...
    if label:
        assert request_body['label'] == label
    if not tag_name:
...
def test_write_log_legacy_tagname(safe_client, tag_name, legacy_tagname):
    ...
    with mock.patch('ligo.gracedb.rest.GraceDb.post') as mock_post, \
         mock.patch(template_prop, mock_template_dict):  # noqa: E127
        safe_client.write_log('TS121212abc', comment, **method_kwargs)
...
def test_write_log_bad_tags(safe_client, tag_name, display_name):
    ...
    with pytest.raises(ValueError, match=err_msg):
        safe_client.write_log('TS121212abc', 'test', tag_name=tag_name,
