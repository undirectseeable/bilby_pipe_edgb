...
from .client import GraceDBClient, DEFAULT_SERVICE_URL, DEFAULT_RETRY_CODES, \
...
class GraceDb(GraceDBClient):
    ...
    Args:
        ...
        backoff_factor (:obj:`float`, optional): The backoff factor for
            retrying. Default: 0.1. Refer to urllib3 documentation for usage.
    ...
    def __init__(self, service_url=DEFAULT_SERVICE_URL,
                 ...
                 reload_certificate=False, reload_buffer=300, use_auth='all',
                 retries=5, retry_codes=DEFAULT_RETRY_CODES,
        ...
        super().__init__(
            ...
            reload_certificate=reload_certificate,
    ...
    def _getCode(self, input_value, code_dict):
        ...
        for code, display in code_dict.items():
    ...
    def create_superevent(self, t_start, t_0, t_end, preferred_event,
        ...
        if not category:
            raise ValueError("category must be one of: {0}".format(
                list(self.superevent_categories.items())))
    ...
    def write_log(self, object_id, message, filename=None, filecontents=None,
                  tag_name=[], displayName=[], label=None, *args, **kwargs):
        ...
        if label:
            if not isinstance(label, str):
                raise TypeError("label arg is {0}, should be "
                                " str".format(type(label)))
            # Check label against those in database
            if label not in self.allowed_labels:
                raise ValueError(("Label '{0}' does not exist in the "
                                 "database").format(label))

        # superevent/event switching logic:
        is_superevent = kwargs.pop('is_superevent', False)
        if is_superevent:
            template = self.templates['superevent-log-list-template']
            uri_kwargs = {'superevent_id': object_id}
    ...
    def write_emobservation(self, object_id, group, raList, raWidthList,
        ...
        if not all(map(lambda lin: len(lin) == num_measurements, all_lists)):
    ...
    @event_or_superevent
    @alias('addTag')
    def add_tag(self, object_id, N, tag_name, displayName=None, *args,
                **kwargs):
    ...
    def create_voevent(self, object_id, voevent_type, skymap_type=None,
        ...
        if not voevent_type:
            raise ValueError("voevent_type must be one of: {0}".format(
                ", ".join(self.voevent_types.values())))
    ...
    def gwtc_create(self, smap, number, comment=None):
        ...
        Args:
           smap (str | dict): path to a json-formatted file containing a
               catalog entry or a dictionary. An example of the format
               is below:
                   {
                     "S231002i": {
                         "pipelines": {
           ...
           comment (str, optional): an analyst comment to accompany the smap
               upload
        ...
        if isinstance(smap, str):
            ...
            raise ValueError(
                "smap must be a string indicating a json filename or "
                "a dictionary"
            )

        # Verify that the number is a string. The server will verify that
        # it is a valid slug.
        ...
        json_request = {"number": number, "smap": smap_json,
                        "comment": comment}

        return self.post(uri, json=json_request)

    def gwtc_get(self, number, version):
        """
        ...
        Args:
            ...
            version (int or str('latest')): positive integer version number
            of the catalog to retrieve. Entering 'latest' will pull the most
            recently uploaded version.

        Returns:
            :class:`requests.models.Response`

            A HTTPresponse with an individual catalog entry, if a specific
        ...
        input_err = "both a number and a version must be supplied to " \
                    "gwtc_get"
        ...
        if number:
            if not isinstance(number, str):
                raise ValueError(f"number must be a str, not {type(number)}")
            elif version or version == 0:  # if user has specified 'version'
                if (
                    ...
                    raise ValueError("version must be a positive integer or "
                                     "'latest'")
                else:
    ...
    def gwtc_list(self, number=None):
        ...
        Note: not specifying a catalog number will retrieve an iterator
        with all available catalogs.
        """

        live_err = f"gwtc functionality is not yet live on {self._service_url}"
        ...
        if number:
            if not isinstance(number, str):
                ...
                try:
                    uri = str(self.templates["gwtc-number-list"]).format(
                        number=number)
                except KeyError:
                    raise NotImplementedError(live_err)
