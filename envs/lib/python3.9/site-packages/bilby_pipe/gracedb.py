...
def calibration_lookup_o4(trigger_time, detector):
    ...
    if detector == "V1":
        ...
        # FIXME: this is a temporary solution that disables the usual
        # calibration lookup for O4 until the issue described in
        # https://git.ligo.org/lscsoft/bilby_pipe/-/issues/307 is addressed
        tmp_cal_dir = "/home/cal/public_html/uncertainty/O4C00tmp/"
        logger.warning(
            (
                "Using temporary calibration uncertainty envelope from "
                f"{tmp_cal_dir} for {detector}"
            )
        )
        return os.path.join(tmp_cal_dir, f"calibration_uncertainty_{detector}_tmp.txt")
...
def calibration_dict_lookup(trigger_time, detectors):
    ...
    try:
        ...
        logger.warning("Could not find valid calibration files.")
        logger.warning("Running without calibration!")
        return None, None
