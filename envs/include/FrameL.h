...
typedef enum {FR_OK,
	      ...
	      FR_ERROR_BAD_NEVENT,
	      FR_ERROR_BAD_NSTAT,
              FR_ERROR_TOO_LONG_STRING,
              FR_ERROR_WRONG_NBYTES} FRERROR;
...
/*  TAI is ahead of UTC by the leaps seconds, see the function FrGetLeapS   */
/*--------------------------------------------------------------------------*/

/* the next define call the FrGetLeapS with the largest possible GPS time */
#define FRGPSLEAPS FrGetLeapS(4294967295)
...
FrBasic*    FrameFindBasic  (FrBasic*root, const char *name);
FrDetector* FrameFindDetector(FrameH *frame,const char *detNameOrPrefix);
FrVect *FrameFindVect   (FrameH *frame, const char *vectName);
FrVect *FrameFindAdcVect(FrameH *frame, const char  *adcName);
FrVect *FrameFindProcVect(FrameH *frame,const char *procName);
FrVect *FrameFindSimVect(FrameH *frame, const char  *simName);
FrVect *FrameFindStatVect(FrameH *frame,const char *statName);
FrVect *FrameFindSumVect(FrameH *frame, const char  *sumName);
void    FrameFree       (FrameH *frame);
FRLONG  FrameGetAdcSize (FrameH *frame);
int     FrameGetLTOffset(FrameH *frame, const char *channelName);
...
void    FrameRemoveUntaggedMsg(FrameH *frame);
...
int     FrameRemoveDuplicatedProc(FrameH *frame);
FrameH* FrameReshape(FrameH* frame, FrameH** workSpace, int newFrameLength);
int     FrameReshapeAdd(FrameH *frame1, FrameH *frame2);
FrameH *FrameReshapeConvertSer(FrameH *trend, FrameH *frame, double trendPeriod);
...
void    FrameSetSuffix(FrameH *frame, char *suffix);
...
FrAdcData *FrAdcDataFind (FrameH *frame, const char *adcName); 
...
FrCList *FrCListBldProc(FrameH *frame);
...
FrEvent *FrEventAddParam(FrEvent *event,  const char *name,  double value);
int      FrEventAddVect (FrEvent *event,  FrVect *vect, const char* newName);
int      FrEventAddVectF(FrEvent *event,  FrVect *vect, const char* newName);

FrEvent *FrEventCopy(FrEvent *event);
FrSH    *FrEventDef(void);
void     FrEventDump(FrEvent *event, FILE *fp, int debugLvl);
FrEvent* FrEventFind(FrameH *frameH, const char *name, FrEvent *last);
FrVect*  FrEventFindVect(FrEvent *event, const char *vectName);
void     FrEventFree(FrEvent *event);
double   FrEventGetParam  (FrEvent *event, const char *paramName);
int      FrEventGetParamId(FrEvent *event, const char *paramName);
FrVect*  FrEventGetVectD(FrEvent *event, const char *vectName);
FrVect*  FrEventGetVectF(FrEvent *event, const char *vectName);
FrEvent *FrEventNew (FrameH *frameH,  
                 const char *name,  const char *comment, const char *inputs, 
                 double gime, float  timeBefore, float  timeAfter,
                 float  amplitude,  float  probability, const char   *stat,
                 FrVect *data, int nParam, ...);
FrEvent *FrEventRead    (FrFile *iFile);
int      FrEventReadData(FrFile *iFile, FrEvent *event);
FrEvent *FrEventReadT   (FrFile *iFile, const char *name, double tStart, 
                  double length, double amplitudeMin, double amplitudeMax);
FrEvent *FrEventReadTF  (FrFile *iFile, const char *name, double tStart, 
                  double length, int readData, int nParam, ...);
FrEvent *FrEventReadTF1(FrFile *iFile, const char *name, double tStart,
                        double length, int readData,  
                        const char *pName1, double min1, double max1);
FrEvent *FrEventReadTF2(FrFile *iFile, const char *name, double tStart,
                        double length, int readData,  
                        const char *pName1, double min1, double max1,
                        const char *pName2, double min2, double max2);
FrEvent *FrEventReadTF3(FrFile *iFile, const char *name, double tStart,
                        double length, int readData,  
                        const char *pName1, double min1, double max1,
                        const char *pName2, double min2, double max2,
                        const char *pName3, double min3, double max3);
FrEvent *FrEventReadTF4(FrFile *iFile, const char *name, double tStart,
                        double length, int readData,  
                        const char *pName1, double min1, double max1,
                        const char *pName2, double min2, double max2,
                        const char *pName3, double min3, double max3,
                        const char *pName4, double min4, double max4);
FrEvent *FrEventReadTF5(FrFile *iFile, const char *name, double tStart,
                        double length, int readData,  
                        const char *pName1, double min1, double max1,
                        const char *pName2, double min2, double max2,
                        const char *pName3, double min3, double max3,
                        const char *pName4, double min4, double max4,
                        const char *pName5, double min5, double max5);
...
FrFileH* FrFileBreakName(const char *fullName, FrFileH *next, FrSegList **segList);
void    FrFileDbg      (FrFile *file);
void    FrFileFree     (FrFile *file);
FrFile *FrFileNew(const char *fileName, int compress, char *buf, FRLONG bufSize);
...
FrVect *FrFileIGetEventInfo (FrFile *iFile, const char *tag,
...
FrVect *FrFileIGetSimEventInfo(FrFile *iFile, const char *tag,
                           double tStart,double len, double aMin, double aMax);
FrVect *FrFileIGetSimEventNames(FrFile *iFile);
FrVect *FrFileIGetSimNames  (FrFile *iFile);
FrVect *FrFileIGetStatNames (FrFile *iFile);

FrVect *FrFileIGetV     (FrFile *iFile, const char *name, double tStart, double len);
FrVect *FrFileIGetVect  (FrFile *iFile, const char *name, double tStart, double len);
FrVect *FrFileIGetVectD (FrFile *iFile, const char *name, double tStart, double len);
FrVect *FrFileIGetVectDN(FrFile *iFile, const char *name, double tStart, double len);
FrVect *FrFileIGetVectF (FrFile *iFile, const char *name, double tStart, double len);
FrVect *FrFileIGetVectFN(FrFile *iFile, const char *name, double tStart, double len);

FrVect *FrFileIGetVAdc (FrFile *iFile, const char *name, double tStart, double len, 
                                       int   group);
FrVect *FrFileIGetVProc(FrFile *iFile, const char *name, double tStart, double len,
                                       int   group);
FrVect *FrFileIGetVSim (FrFile *iFile, const char *name, double tStart, double len,
		                       int   group);
FrVect *FrFileIGetVType(FrFile *iFile, const char *name, double tStart, double len,
...
FRLONG  FrFileIOTell(FrFile *iFile);
...
FRLONG  FrFileOEnd(FrFile *file);
FrFile *FrFileONew (const char *fileName, int compress);
FrFile *FrFileONewH(const char *fileName, int compress, const char *program);
FrFile *FrFileONewFd   (FrIO *frfd, int compress);
FrFile *FrFileONewFdH  (FrIO *frfd, int compress, const char *program);
FrFile *FrFileONewM(const char *path,     int compress, const char *program, int length);
FrFile *FrFileONewMD(const char* path,    int compress, const char* program, int fLength,
		     const char* filePrefix, int   dirPeriod);

FrFileH* FrFileOpenCacheFile(const char   *fullName);
FrFileH** FrFileOpenFFL(const char *name, FrFileH **root, FrSegList** segList);
void    FrFileOOpen        (FrFile *oFile);
int     FrFileORealloc     (FrFile *oFile, char *name, FRLONG size);
...
struct FrFileH{                  /* header for each file                     */
  ...
  char  *iMBuffer;               /* buffer if the file is written in memory  */
  int    iMBSize;                /* number of bytes of iMBuffer              */
...
FrProcData *FrProcDataFind (FrameH *frame, const char *name);
...
FrSegList *FrSegListNew(const char *name);
int        FrSegListPrint     (FrSegList *segList, char *fileName);
FrSegList* FrSegListRead(const char *fileName);
  FrSegList* FrSegListReadFP(FILE *fp, const char* name);
...
FrSerData *FrSerDataNew (FrameH *frame, char *smsName, 
                         double serTime, char *data, double sampleRate);
...
FrSimData *FrSimDataFind (FrameH *frame, const char *name);
...
FrSimEvent *FrSimEventFind(FrameH *frameH, const char *name, FrSimEvent *last);
FrVect*     FrSimEventFindVect  (FrSimEvent *event, const char *vectName);
void        FrSimEventFree      (FrSimEvent *event);
double      FrSimEventGetParam  (FrSimEvent *event, const char *paramName);
int         FrSimEventGetParamId(FrSimEvent *event, const char *paramName);
FrVect*     FrSimEventGetVectD  (FrSimEvent *event, const char *vectName);
FrVect*     FrSimEventGetVectF  (FrSimEvent *event, const char *vectName);
FrSimEvent *FrSimEventNew(FrameH *frameH, const char *name, char *comment,
...
FrSimEvent *FrSimEventReadT(FrFile *iFile, const char *name,
                            double gtime, double dt, double aMin, double aMax);
FrSimEvent *FrSimEventReadTF  (FrFile *iFile, const char *name, double tStart, 
                        double length, int readData, int nParam, ...);
FrSimEvent *FrSimEventReadTF1(FrFile *iFile, const char *name, double tStart,
                        double length, int readData,  
                        const char *pName1, double min1, double max1);
...
void        FrSimEventUpdateTime(FrSimEvent *evt, double dt);
...
FrStatData *FrStatDataFind(FrDetector *detector, const char *name,
...
FrSummary *FrSummaryFind (FrameH *frame, const char *name);
...
char*  FrTagGetSubTag(const char *type, const char *tag);
FRBOOL FrTagMatch(FrTag *tag, const char *name);
FRBOOL FrTagMatch1(const char *name, int   nameL, const char *tag, int   tagL);
FrTag *FrTagNew(const char *tag);
...
int    FrTOCGetLTOffset(FrFile *iFile, const char *channelName);
...
void      FrTOCevtDump(FrTOCevt *evt, FILE *fp, char *tag, int maxPrint, int ULeapS);
void      FrTOCevtFree(FrTOCevt *evt);
FrVect   *FrTOCevtGetEventInfo(FrTOCevt *root, const char *tag,
                               double tStart, double length,
...
FrVect *FrVectCopyToD   (FrVect *vect, double scale, const char *newName);
FrVect *FrVectCopyToF   (FrVect *vect, double scale, const char *newName);
FrVect *FrVectCopyToI   (FrVect *vect, double scale, const char *newName);
FrVect *FrVectCopyToS   (FrVect *vect, double scale, const char *newName);
FrVect *FrVectCopyToType(FrVect *vect, double scale, const char *newName, int type);
...
#define FRVECTFREE(a) FrVectFree(a); a = NULL;
...
double  FrVectMin    (FrVect *vect);
int     FrVectMinMax (FrVect *vect, double *min, double *max);
FrVect *FrVectNew  (int type, int nDim, ...);
FrVect *FrVectNewL (int type, int nDim, ...);
FrVect *FrVectNewTS(const char *name,double sampleRate, FRLONG nData, int nBits);
FrVect *FrVectNew1D(const char *name,int type, FRLONG nData, 
                           double dx, const char *unitx, const char *unity);
FrVect *FrVectRead    (FrFile *iFile);
FrVect *FrVectReadNext(FrFile *iFile, double gtime, const char *name);
...
void    FrVectSetName    (FrVect *vect, const char *name);
void    FrVectSetUnitX   (FrVect *vect, const char *unitX);
void    FrVectSetUnitY   (FrVect *vect, const char *unitY);
...
/*---------------------------------------------------------------------------*/
FrSH *FrDicAddS(char *type, void (*funRead)(void));         
...
char  *FrStrCpy(char **copy, const char *old);
char  *FrStrGTime(unsigned int in);
int    FrStrcmpAndPrefix(const char *name1, const char* name2);
char*  FrStrFlipPrefix(const char *name);
int    FrStrSetPrefix(char **name, const char *prefix);
int    FrStrSetSuffix(char **name, const char *suffix);
char  *FrStrUTC(unsigned int gps, int uLeapS);
char*  FrGPS2GWname(int gps, char* name, int nameSize);
...
void FrPutVC(FrFile *oFile, char      *data, FRLONG nData);
void FrPutVD(FrFile *oFile, double    *data, FRLONG nData);
void FrPutVF(FrFile *oFile, float     *data, FRLONG nData);
void FrPutVFD(FrFile *oFile,double    *data, FRLONG nData);
void FrPutVI(FrFile *oFile, int       *data, FRLONG nData);
void FrPutVL(FrFile *oFile, FRLONG    *data, FRLONG nData);
void FrPutVQ(FrFile *oFile, char     **data, FRLONG nData);
void FrPutVS(FrFile *oFile, short     *data, FRLONG nData);
